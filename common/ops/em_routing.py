...
def em_routing(votes_ij, activations_i, beta_a, beta_v, iter_routing, final_lambda, epsilon, spatial_routing_matrix):
...
# AG 26/06/2018: added var_j
def e_step(votes_ij, activations_j, mean_j, stdv_j, var_j, spatial_routing_matrix, epsilon):
  ...
  with tf.name_scope("e_step") as scope:
    ...
    with tf.name_scope("softmax_across_parents") as scope:
      zz_softmax = softmax_across_parents(zz, spatial_routing_matrix)
...
def softmax_across_parents(probs_sparse, spatial_routing_matrix):
  ...
  parent_softmax = tf.nn.softmax(probs_sparse, axis=-1)
