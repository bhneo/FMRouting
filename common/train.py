...
class Trainer(object):
    def __init__(self, model, params, data_info, monitor=None, bar=True):
        self.model = model
        self.params = params
    ...
    def get_train_step(self):
        ...
        def train_step(images, labels):
            with tf.GradientTape() as tape:
                ...
                loss = self.get_loss(labels, predictions)

            gradients = tape.gradient(loss, self.model.trainable_variables)
            self.model.optimizer.apply_gradients(zip(gradients, self.model.trainable_variables))

            self.update_metrics(loss, labels, predictions)
        return train_step
    ...
    def log_tensors(self, batch, images, labels, writer, epoch):
        if batch == 0 and self.monitor is not None:
            logs = self.monitor.model((images, labels))
    ...
    def train_one_epoch(self, train_set, train_writer, epoch):
        ...
        for batch, (images, labels) in enumerate(train_set):
            ...
            self.log_tensors(batch, images, labels, train_writer, epoch)
